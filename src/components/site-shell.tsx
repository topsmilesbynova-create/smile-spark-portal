    ...
    <header className="fixed inset-x-0 top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Main navigation">
          {links.map(([label, to]) => to.includes("#") ? <a key={label} href={to} className="text-sm text-muted-foreground transition-colors hover:text-foreground">{label}</a> : <Link key={label} to={to as "/about"} className="text-sm text-muted-foreground transition-colors hover:text-foreground" activeProps={{ className: "text-foreground" }}>{label}</Link>)}
        </nav>
      ...
      {open && <nav className="border-t border-border bg-background px-5 py-5 lg:hidden">{links.map(([label, to]) => to.includes("#") ? <a key={label} href={to} onClick={() => setOpen(false)} className="block border-b border-border py-3 text-sm">{label}</a> : <Link key={label} to={to as "/about"} onClick={() => setOpen(false)} className="block border-b border-border py-3 text-sm">{label}</Link>)}<Button asChild className="mt-5 w-full" size="lg"><Link to="/book" onClick={() => setOpen(false)}>Book a consultation</Link></Button></nav>}
    </header>
